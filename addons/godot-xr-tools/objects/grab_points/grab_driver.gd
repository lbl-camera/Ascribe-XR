...
func _physics_process(delta : float) -> void:
    ...
    if is_instance_valid(secondary):
        ...
        var target_child = target.get_child(0)
        var target_scale = primary.by.global_position.distance_to(secondary.by.global_position)
        target_child.scale = Vector3(target_scale, target_scale, target_scale)
